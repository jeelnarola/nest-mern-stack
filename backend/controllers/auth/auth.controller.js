...
const user = await User.findOne({
         ...
         if (user) {
            ...
            else if (user.phone === phone) message = "Mobile number is already in use!";
            return res.status(400).json({ success: false, message });
        }
        const newUser = new User({
            firstName,lastName,email,password,phone,role,address
        })
        ...
        console.log("Error By auth controller in register function :- ",error.message);
